...
export class Client {
  ...
  private onRecvMessage = async (body: string) => {
    ...
    try {
      ...
      logger.debug(
        `onRecvMessage. uid:${this.session.uid}, type:${msg.type}, data:${msg.data}, result:${result}, elapsed:${
          end - begin
        }ms`,
      );
    } catch (ex) {
  ...
  onZoneMessage(type: ActionTypes, req: ZoneCmdReq) {
    ...
    switch (type) {
      ...
      case ActionTypes.ZONE_COMMAND_SERVO_ON:
        ...
        break;
      case ActionTypes.ZONE_COMMAND_ONLINE:
        Service.Inst.Redis.publish(
          `TCMCmdCh:${Math.floor(req.ZoneID / 100)}`,
          MsgUtils.makeUimZoneStateChangeReq(req.ZoneID, 'InService'),
        );
        break;
      case ActionTypes.ZONE_COMMAND_OFFLINE:
        Service.Inst.Redis.publish(
          `TCMCmdCh:${Math.floor(req.ZoneID / 100)}`,
          MsgUtils.makeUimZoneStateChangeReq(req.ZoneID, 'Offline'),
        );
        break;
      case ActionTypes.ZONE_COMMAND_AUTO:
        Service.Inst.Redis.publish(
          `TCMCmdCh:${Math.floor(req.ZoneID / 100)}`,
          MsgUtils.makeMessageUimZoneUserAttributes(1, -1, req.ZoneID),
        );
        break;
      case ActionTypes.ZONE_COMMAND_MANUAL:
        Service.Inst.Redis.publish(
          `TCMCmdCh:${Math.floor(req.ZoneID / 100)}`,
          MsgUtils.makeMessageUimZoneUserAttributes(0, -1, req.ZoneID),
        );
        break;
      case ActionTypes.ZONE_COMMAND_SET_LIMIT:
      ...
      case ActionTypes.HIM_FORCE_INSTALL_CARRIER:
        Service.Inst.Redis.publish(
          `TCMCmdCh:${Math.floor(req.ZoneID / 100)}`,
          MsgUtils.makeHimForceInstallCarrier(req.ZoneID, req.CarrierID),
        );
        break;
      case ActionTypes.ZONE_COMMAND_CHANGE_LOCATION:
      ...
      case ActionTypes.ZONE_COMMAND_DEFAULT_DESTNATION:
        Service.Inst.Redis.publish(
          `TCMCmdCh:${Math.floor(req.ZoneID / 100)}`,
          MsgUtils.makeMessageUimZoneUserAttributes(-1, req.DefaultDest, req.ZoneID),
        );
        break;
      case ActionTypes.READ_RFID:
